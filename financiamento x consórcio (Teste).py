...
def financiamento_simples(valor_financiado, prazo, taxa_mensal, modelo, amortizacao):
    ...
    if modelo == "Price":
        ...
        parcela = (saldo + juros_total) / prazo

        for m in range(1, prazo + 1):
            juros = saldo * taxa_mensal
            amort = parcela - juros + amortizacao
            saldo -= amort
            dados.append([m, parcela + amortizacao, juros, amort, max(saldo, 0)])
...
def calcular_consorcio(valor_credito, prazo, taxa_adm, fundo_reserva, lance_pct, prazo_contemplacao):
...
with tab_c:
    ...
    with c1:
        ...
        lance_pct = st.slider("Lance (%)", 0.0, 100.0, 30.0)
        prazo_contemplacao = st.number_input("Prazo estimado de contemplação (meses)", 1, prazo_c, 12)

    res_c = calcular_consorcio(
        valor_credito, prazo_c, taxa_adm, fundo_reserva, lance_pct, prazo_contemplacao
    )

    with c2:
        st.subheader("📌 Cenário Pré-Contemplação")
        st.markdown(f"""
        <div class="card">
        • Parcela mensal: <b>R$ {res_c['Parcela']:,.2f}</b><br>
        • Pagamento até contemplação: <b>{prazo_contemplacao} meses</b><br>
        • Total pago até contemplação: <b>R$ {res_c['Parcela'] * prazo_contemplacao:,.2f}</b>
        </div>
        """, unsafe_allow_html=True)

        st.subheader("🚀 Cenário Pós-Contemplação")
        st.markdown(f"""
        <div class="card">
        • Crédito liberado: <b>R$ {valor_credito:,.2f}</b><br>
        • Lance ofertado: <b>R$ {res_c['Lance']:,.2f}</b><br>
        • Saldo de parcelas restantes: <b>{prazo_c - prazo_contemplacao} meses</b>
        </div>
        """, unsafe_allow_html=True)
